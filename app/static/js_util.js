...
// From here on: Author: jhorstmann
...
/**
 * Default Userdialog if the operation fails.
 * @param {*} request
 * @param {*} status
 * @param {*} errorThrown
 */
function defaultUserFail(request, status, errorThrown) {
  console.log(request, status, errorThrown);
  var json_data = request.responseJSON;

  if (!json_data){
    alert(this.url + " responded with " + request.status,
    errorThrown,
    "\n",
    request.responseText);
  } else {
