...
function createMessenger(imageObj) {
  ...
  for (let message of messages) {
    ...
    if (!message.startsWith("**sys**")) {
      messengerHTML +=
        ...
        `<label for="${
          message["id"]
        }" class="col-sm-2 col-form-label"><span style="color: blue; ">${
          message["user"]["email"]
        }</span> ${formatDate(message["date"])}</label>\n` +
        '    <div class="col-sm-10">\n' +
      ...
      messengerHTML +=
        ...
        `<label for="${
          message["id"]
        }" class="col-sm-2 col-form-label"><span style="color: green; ">${
          message["user"]["email"]
        }</span> ${formatDate(message["date"])}</label>\n` +
        '    <div class="col-sm-10">\n' +
        `        <textarea id="${message["id"]}" class=\"form-control\" disabled>${message["message"]}</textarea>\n` +
  ...
  // Submit button
  messengerHTML +=
    '<button class="btn btn-primary" name="accept-btn" id="send-message-btn">Send</button>';
